	...
	def parse_tcf(self, fname):
		...
		for i in data.split("\n"):
			...
			if i.startswith(".device"):
				...
				current_tile = self.tile_dict.get(tile_name)
				if current_tile is None:
					# DCU/SERDES/PLL tiles not in db parser; not part of logic. Skip.
					print("SKIP unknown tile: " + tile_name)
...
if __name__ == "__main__":
	import sys
	src = sys.argv[1] if len(sys.argv) > 1 else "example/counter.tcf"
	out = sys.argv[2] if len(sys.argv) > 2 else "example/counter_decomp.v"
	ecp5 = ECP5(src)
	ecp5.write_code(out)
